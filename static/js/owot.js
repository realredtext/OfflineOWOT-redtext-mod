...
var images_to_load = {
	// (MOD)
    unloaded: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAAAAACMmsGiAAAAAklEQVR4AewaftIAAAAYSURBVAXBgQAAAADDoPsDRTWI14pFo6YcgrQNEaFuVVoAAAAASUVORK5CYII="
};
...
function event_keydown(e) {
    ...
    if(checkKeyPress(e, keyConfig.tab)) { // tab
        for(var i = 0; i < tabLength; i++) writeChar(" ");
        e.preventDefault();
    }
    w.emit("keyDown", e);
}
...
function int_to_hexcode(value, mode="long") {
...
function buildMenu() {
    ...
    if (Permissions.can_admin(state.userModel, state.worldModel) || state.userModel.is_operator) {
        menu.addOption("Make an area owner-only", () => {
            return w.doProtect("owner-only");
        });
    }
    if (Permissions.can_protect_tiles(state.userModel, state.worldModel) || USER_LEVEL === 1) {
...
var network = {
    ...
    protect: function(position, type="protect", callback) {
...
Object.assign(w, {
    ...
    doGoToCoord: function(y, x) {
        var maxX = 14073748835532; // do not go beyond these coords, or 2251799813685246
...
var ws_functions = {
    ...
    ping: function(data) {
        if(data.time) {
            ...
            serverChatResponse(`Ping: ${pingMs} MS`);
            return;
        }
    },
...
let tile_storage = {
    save: () => {
        localStorage.setItem("tile_save", JSON.stringify(tiles));
    },

    access: () => {
        let tl = JSON.parse(localStorage.getItem("tile_save"));
        tiles = tl;
        w.redraw();
    },

    delete: () => {
        let conf = confirm("Are you sure you want to wipe your tile save?");
        if(conf) {
            localStorage.removeItem("tile_save");
            tiles = {};
        }
    }
}
